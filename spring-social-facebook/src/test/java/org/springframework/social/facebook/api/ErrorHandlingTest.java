...
public class ErrorHandlingTest extends AbstractFacebookApiTest {
	...
	public void unknownAlias() {
		try {
			mockServer.expect(requestTo("https://graph.facebook.com/v1.0/dummyalias"))
	...
	public void unknownPath() {
		try {
			mockServer.expect(requestTo("https://graph.facebook.com/v1.0/me/boguspath"))
				.andExpect(method(GET))
				.andExpect(header("Authorization", "OAuth someAccessToken"))
	...
	public void resource_noAccessToken() {
		...
		mockServer.expect(requestTo("https://graph.facebook.com/v1.0/me"))
			.andExpect(method(GET))
	...
	public void falseResponse() {
		mockServer.expect(requestTo("https://graph.facebook.com/v1.0/someobject"))
			.andExpect(method(GET))
			.andExpect(header("Authorization", "OAuth someAccessToken"))
	...
	public void notFound() {
		mockServer.expect(requestTo("https://graph.facebook.com/v1.0/nobody/feed?limit=25"))
			.andExpect(method(GET))
			.andExpect(header("Authorization", "OAuth someAccessToken"))
			.andRespond(withStatus(HttpStatus.NOT_FOUND).body(jsonResource("error-404-unknown-alias")).contentType(MediaType.APPLICATION_JSON));
	...
	public void notAFriend() {
		...
		try {
			mockServer.expect(requestTo("https://graph.facebook.com/v1.0/119297590579/members/100001387295207"))
				.andExpect(method(POST))
				.andExpect(header("Authorization", "OAuth someAccessToken"))
	...
	public void notTheOwner() {
		...
		try {
			mockServer.expect(requestTo("https://graph.facebook.com/v1.0/1234567890"))
				.andExpect(method(POST))
				.andExpect(header("Authorization", "OAuth someAccessToken"))
				.andExpect(content().string("method=delete"))
